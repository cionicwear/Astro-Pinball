...
class Ball {
  ...
  isCollidedWithLine(obj){
    ...
    if ((distY <= obj.halfheight) && (this.bouncedTwo === false)) {
      this.changeBounce();
      return true;
    }
  ...
  /**
   *
   * @param {obj} a {x, y, height, width}
   * @param {obj} b {x, y, height, width}
   */
  isCollide(a, b) {
    if (isNaN(a.x) || isNaN(b.x) || isNaN(a.y) || isNaN(a.height) || isNaN(a.width) || isNaN(b.y) || isNaN(b.width) || isNaN(b.height)) return false;
    console.log(a,b);
    return !(
      ((a.y + a.height) < (b.y)) ||
      (a.y > (b.y + b.height)) ||
  ...
  hitbackBumper(obj){
    ...
    this.ballVelX = (this.refl.x/length) * this.speed * 1.04;
  ...
  isCollidedwithSideBump(obj){
    ...
    // Distance too far
    if (distX > (obj.halfwidthTwo + this.radius + 0.2*distX)) { return false; }
    if (distY > (obj.halfheightTwo + this.radius + 0.2*distY)) { return false; }
